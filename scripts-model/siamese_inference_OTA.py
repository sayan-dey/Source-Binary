...
def get_topK_scores(args, csv_filepath):
    ...
    data_stacked = np.vstack(d['embed_O_i2v'].values) # Stack all arrays in the 'embed_O_i2v' column vertically to create a 2D array
    ...
    for index,row in d.iterrows():
        data_v2v.append((row['key_v2v'], torch.from_numpy(row['embed_O_v2v']), torch.from_numpy(row['embed_T_v2v']), torch.from_numpy(row['embed_A_v2v']), torch.from_numpy(row['strembed_v2v']), torch.from_numpy(row['libemb_v2v'])))
        data_i2v.append((row['key_i2v'], torch.from_numpy(row['embed_O_i2v']), torch.from_numpy(row['embed_T_i2v']), torch.from_numpy(row['embed_A_i2v']), torch.from_numpy(row['strembed_i2v']), torch.from_numpy(row['libemb_i2v'])))
    ...
    for _, data in enumerate(v2v_dataloader):
        ...
        with torch.no_grad():
            output, attn_weights = model(data[1].float().to(device), data[2].float().to(device), data[3].float().to(device), data[4].float().to(device), data[5].float().to(device))
