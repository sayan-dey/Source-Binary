...
#Eg cmd: python siamese_inference.py 
# -bmp /Pramana/VexIR2Vec/Source_Binary/models/COFO-model-with-8-pass-seqs-train-data-noRelu/cont_adam_lr0.0001_b32_e500_m0.04_128D_b0.9.model 
# -dp na -csv_filepath /Pramana/VexIR2Vec/Source_Binary/COFO-Dataset/C_data_infer_only_test.csv 
# -out_dir abc -res_dir /Pramana/VexIR2Vec/Source_Binary/inference/ > /Pramana/VexIR2Vec/Source_Binary/test/test2.txt
...
def get_topK_scores(args, csv_filepath):
    ...
    for index,row in d.iterrows():
        data_v2v.append((row['key_v2v'], torch.from_numpy(row['embed_v2v'])))
        data_i2v.append((row['key_i2v'], torch.from_numpy(row['embed_i2v'])))
    ...
    for _, data in enumerate(v2v_dataloader):
        ...
        with torch.no_grad():
            output = model(data[1].float().to(device))
