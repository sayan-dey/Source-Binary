# Need to complete it and update eg running cmd...

# Script for parallely generating angr-vex db files from .out files in COFO-Dataset 

# Sample cmd:
# python gen_database.py -vocab /Pramana/VexIR2Vec/checkpoint/ckpt_3M_900E_128D_0.0002LR_adam/seedEmbedding_3M_900E_128D_0.0002LR_adam -angr_path /Pramana/VexIR2Vec/vexIR-repo-sayan/angr-vex -fchunks 1 -num_threads 12 -adb na
...
def execute_angr_command(root, max_workers):
    ...
    for compiler in compilers:
        for pass_seq in range(1,9):
            
            print(f"\nRunning for {compiler} and pass seq {pass_seq}...\n")
            ...
            for dirpath, _, filenames in os.walk(root):
                if dirpath.endswith('submissions'):
                    ...
                    if os.path.exists(c_directory) and c_directory in data_dict["classes_test"]: #for only generating infer set
                        # config_dir = os.path.join(c_directory, f'x86-{compiler}-{opt}')
                        # config_dir = os.path.join(c_directory, f'{compiler}-{opt}')
                        
                        config_dir = os.path.join(c_directory, f'{compiler}-pass-seq-{pass_seq}')
...
parser.add_argument("-adb", "--angr_db_path", type=str, help="Path to store Angr Proj db")
