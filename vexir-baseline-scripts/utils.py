...
NUM_WALKS = 1
NUM_NEIGHBORS=10
CNN_INP_DIM = 150 #here, it means 80x80
...
def preprocess_CSV_dataset(csv_filepath, pca_model_path):
    ...
    for idx in range(len(d)):
        ...
        if row.iloc[0][7] == 1: #similar
            #strembed_v2v,libemb_v2v,embed_v2v,strembed_i2v,libemb_i2v,embed_i2v,label
            pos_pairs.append((torch.from_numpy(row.iloc[0][1]), torch.from_numpy(row.iloc[0][2]), torch.from_numpy(row.iloc[0][3]), torch.from_numpy(row.iloc[0][5]), torch.from_numpy(row.iloc[0][8]), torch.from_numpy(row.iloc[0][6]), torch.from_numpy(np.array(1))))
            
        else:  #dissimilar
            #strembed_v2v,libemb_v2v,embed_v2v,strembed_i2v,libemb_i2v,embed_i2v,label
            neg_pairs.append((torch.from_numpy(row.iloc[0][1]), torch.from_numpy(row.iloc[0][2]), torch.from_numpy(row.iloc[0][3]), torch.from_numpy(row.iloc[0][5]), torch.from_numpy(row.iloc[0][8]), torch.from_numpy(row.iloc[0][6]), torch.from_numpy(np.array(0))))
